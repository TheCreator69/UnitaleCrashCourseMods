...
function EncounterStarting()
    Inventory.AddCustomItems({"Bun", "Another Bun"}, {0, 0})
    Inventory.SetInventory({"Bun", "Another Bun"})
    -- If you want to change the game state immediately, this is the place.
end

function HandleItem(ItemID)
    lastItem = Inventory.GetItem(1 + Inventory.ItemCount)
    BattleDialog({"Last item: " .. lastItem})
end
